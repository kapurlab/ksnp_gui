# kSNP4 — OOD sandbox app (Slurm, no admin)

Per-user deployment of kSNP4 on any Open OnDemand HPC. The card appears
...
bin/bdtools install --sandbox ksnp_gui        # builds env+frontend, links this card
$EDITOR ~/.../ood/apps/ksnp_gui_sandbox/form.yml   # set cluster: "CHANGE_ME" -> your cluster
```

`bdtools install --sandbox ksnp_gui` writes `~/.config/ksnp_gui/sandbox.env`
(`BDTOOLS_APP_DIR` + `BDTOOLS_APP_ENV`) and symlinks this card into
`~/ondemand/dev/ksnp_gui`. The launcher (`template/script.sh.erb`) sources
