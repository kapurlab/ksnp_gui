...
def _rp_metrics(run_dir: Path) -> Dict[str, Any]:
    """kSNP is run-centric: one row per RUN, not per sample."""
    d = _rp_json(run_dir, "run_manifest.json")
    res = d.get("results") or {}
    kch = d.get("kchooser") or {}
    opts = d.get("options") or {}
    return {
        "genomes": d.get("genome_count") or None,
        "k": kch.get("optimum_k") or opts.get("k") or None,
        "snps": res.get("snps_all"),
        "core": res.get("core_snps"),
    }
...
_RP_EXPORT_COLUMNS = [("sample", "Run"), ("status", "Status"),
                      ...
                      ("qc_reasons", "QC notes")] + [("genomes", "Genomes"), ("k", "k"), ("snps", "SNPs"), ("core", "Core SNPs")] + \
...
def api_project_results_csv(name: str, start: str = Query(""), end: str = Query(""),
    ...
    return Response(content=buf.getvalue(), media_type="text/csv",
                    headers={"Content-Disposition":
                             'attachment; filename="%s_ksnp_results.csv"' % name})
...
def api_project_results_xlsx(name: str, start: str = Query(""), end: str = Query(""),
    ...
    return Response(content=buf.getvalue(),
                    ...
                    headers={"Content-Disposition":
                             'attachment; filename="%s_ksnp_results.xlsx"' % name})
